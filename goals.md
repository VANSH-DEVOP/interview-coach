...
- [x] ~~**Follow-up:** no coverage measurement or threshold yet.~~ ✅ 2026-08-11 — backend **91%** (branch coverage on) with `fail_under = 90` in `pyproject.toml`; frontend **19%** with floors in `vitest.config.mts`. Both gate CI and both exit non-zero when breached (verified, not assumed).
  Thresholds set from measurement and just below it: a ratchet against regression, not an aspiration that fails on day one.
  `--cov` is deliberately not in `addopts` — running a single test file measures the whole app at ~41% and would fail the threshold every time.
  **The frontend's headline number was a lie until `coverage.all: true`.** v8 counts only files a test imported, which scored this project 86% — 86% of the three tested files, with every page and component missing from the denominator. The honest figure is 19%, and the flattering one would have *fallen* whenever someone added the first test for a page. The frontend floors guard against tests being deleted; they are not a claim that it is tested.
- [x] ~~**Follow-up:** no dependency scanning. `npm audit` reports pre-existing high findings in `next`, `postcss`, and `sharp`.~~ ✅ 2026-08-11 — `pip-audit` and `npm audit --audit-level=high` gate CI, plus `.github/dependabot.yml` for pip, npm, GitHub Actions and both Dockerfiles. Both trees are now **clean**.
  All three npm findings had one root: `postcss@8.4.31` and `sharp@0.34.5` are bundled *inside* Next's own tree (the top-level `postcss` was already patched), and the advisory range covered every 15.x. **Upgraded Next 15.5.19 → 16.3.0.** Verified beyond the build: typecheck, 53 tests, and — because Next 16 renames middleware to "Proxy" — a running production server re-checked for the CSP nonce (0 un-nonced inline scripts, 14 tags carrying the header's nonce) and the route guard (307).
  Backend had 8 findings across 4 packages. **pypdf was the one that mattered**: a crafted PDF causing memory exhaustion *during text extraction*, which is exactly what `ResumeParser.parse` does to any registered user's upload. Upgraded, along with pydantic-settings.
...
- [x] ~~**httpOnly cookie BFF for tokens.**~~ ✅ 2026-08-11 — `frontend/src/app/api/bff/[...path]/route.ts`. The browser no longer holds a token: sessions are httpOnly cookies and `api-client.ts` sends no `Authorization` header at all.
  **Every** call is proxied, not just the auth ones — anything going straight to the API would need a header, which means JavaScript would need the token, which is the thing being removed.
  Token pairs are caught **by shape rather than by path**, so login, refresh, password reset and whatever is added next cannot leak by being forgotten in a list.
  **Refresh moved server-side with its three outcomes intact**: unreachable or 5xx answers 503 with the cookies untouched; only a genuine rejection ends the session. Collapsing those was the old bug where an outage rendered as a logout.
  Verified live against a running stack, not only against mocks: login returns `{"token_type":"bearer"}` with the pair in `HttpOnly; Secure; SameSite=lax` cookies, `/users/me` succeeds on the cookie alone, cross-origin is refused, logout ends it, and a corrupted access token produces `users/me → auth/refresh → users/me` with rotated cookies.
  **Knock-on:** CSP `connect-src` tightened to `'self'`. The `force-dynamic` nonce CSP was re-examined as promised and **kept** — an injected script can no longer steal the session, but it can still act as the user through the proxy and read the page.
...
- [x] ~~**Metrics** — nothing scrapes, aggregates or alerts on them.~~ ✅ 2026-08-11 — `GET /metrics` in Prometheus exposition, off until `METRICS_ENABLED` and token-guarded.
  **Exports the existing counters rather than replacing them.** A collector reads the five modules' snapshots at scrape time, so `/health` and `/metrics` cannot disagree and no module grew a second write path that could fail inside a request. Process-local state, which is a caveat at `/health`, is exactly what a per-instance scrape wants.
  Two conventions with tests behind them: **raw counters, never rates** (`fallback_rate` is asserted *absent* — a lifetime average cannot be alerted on), and **route templates, never raw paths** (on a 404 the label is attacker-controlled, and a few thousand random URLs would exhaust the scrape).
  **Not covered:** the worker runs no HTTP server, so its counters are unscrapable. Its liveness is already the Redis heartbeat and `/health`'s `worker` block; exporting the rest needs a pushgateway, which is its own decision.
...
- [x] ~~**Streaming responses** for question generation and evaluation.~~ ✅ 2026-08-11 (partial, and the partiality is the finding) — `ModelClient.stream_json()`, `AI_STREAMING`, plus `first_token_ms` telemetry at `/health`.
  **Streaming cannot go where the roadmap assumed.** Question generation runs generate → critique → refine, so a streamed set would be rewritten under the reader; evaluation is queued to a worker with nobody watching. `follow_up` — one call, no critique, user waiting — is the only path where it is both safe and useful, and is the only one wired.
  The method returns the *same* parsed JSON and raises the *same* error as the buffered one, deliberately: a streaming path with weaker guarantees is how a faster code path becomes the one that ships broken output.
  **Remaining half: the HTTP surface**, blocked on the session model rather than on effort. A `StreamingResponse` body runs after the route returns, when the request-scoped session has already committed and closed, so persisting a follow-up mid-stream needs a second hand-rolled transaction. Its own design pass, not a bolt-on.
